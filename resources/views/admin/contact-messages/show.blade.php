...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <a href="{{ route('admin.contact-messages.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-800 mb-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-indigo-100">{{ $message->email }}</p>
                                @if($message->phone)
                                    <p class="text-indigo-100">{{ $message->phone }}</p>
                                @endif
                            </div>
                        </div>
                        @php
                        ...
                        @endphp
                        <span class="px-4 py-2 {{ $statusColors[$message->status] }} text-white text-sm font-semibold rounded-full">
                ...
                @if($message->admin_notes)
                    <div class="px-6 py-4 bg-yellow-50 border-t border-yellow-200">
                        ...
                        <p class="text-gray-700 whitespace-pre-wrap">{{ $message->admin_notes }}</p>
                    </div>
                @endif

                <!-- Reply Info -->
                @if($message->status === 'replied' && $message->repliedBy)
                    <div class="px-6 py-4 bg-green-50 border-t border-green-200">
                        <div class="flex items-center gap-2 text-sm">
                            ...
                            <span class="text-gray-700">
                                Replied by <strong>{{ $message->repliedBy->name }}</strong> on {{ $message->updated_at->format('F d, Y \a\t h:i A') }}
                            </span>
                        </div>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <form action="{{ route('admin.contact-messages.reply', $message) }}" method="POST" class="p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="reply_subject" name="subject" value="Re: {{ $message->subject }}" required
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="reply_message" name="reply_message" rows="8" required
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                                  placeholder="Type your reply here...">{{ old('reply_message', "Hi {$message->name},\n\nThank you for contacting us.\n\n") }}</textarea>
                        @error('reply_message')
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="space-y-3">
                    <a href="mailto:{{ $message->email }}" class="w-full px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-medium rounded-lg transition inline-flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($message->phone)
                        <a href="tel:{{ $message->phone }}" class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition inline-flex items-center justify-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <dl class="space-y-3">
                    <div>
                        ...
                        <dd class="text-sm text-gray-900 mt-1">{{ $message->created_at->format('M d, Y h:i A') }}</dd>
                    </div>
                    @if($message->read_at)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">First Read</dt>
                            <dd class="text-sm text-gray-900 mt-1">{{ $message->read_at->format('M d, Y h:i A') }}</dd>
                        </div>
                    @endif
                    <div>
                        <dt class="text-sm font-medium text-gray-500">Last Updated</dt>
                        <dd class="text-sm text-gray-900 mt-1">{{ $message->updated_at->format('M d, Y h:i A') }}</dd>
                    </div>
                </dl>
            </div>
        </div>
...
function insertTemplate(templateType) {
    ...
    const phone = '{{ $message->phone ?? "[Your Phone]" }}';
